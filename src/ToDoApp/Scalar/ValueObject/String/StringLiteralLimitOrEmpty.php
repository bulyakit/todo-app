...
use ToDoApp\Scalar\Exception\InvalidLengthException;
use ToDoApp\Scalar\Exception\InvalidStringException;

/**
 * Class StringLiteralLimitOrEmpty
 *
 * @package ToDoApp\Scalar
 */
class StringLiteralLimitOrEmpty extends StringLiteral
{
    /**
    ...
    public static function validate($value)
    {
        ...
        $length = strlen($value);
        if ($value !== '' && !is_null(static::MIN_LENGTH) && $length < static::MIN_LENGTH) {
            throw new InvalidLengthException(
                'The string is too short [value: ' . $value . ', minLength: ' . static::MIN_LENGTH . ', '
                . 'maxLength: ' . static::MAX_LENGTH . ']'
            );
        }
        if ($value !== '' && !is_null(static::MAX_LENGTH) && $length > static::MAX_LENGTH) {
            throw new InvalidLengthException(
